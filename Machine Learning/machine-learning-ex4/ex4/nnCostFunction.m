...
%unregularised cost
J=sum(sum(1/m*sum(((-y_i).*log(a3)-(1-y_i).*log(1-a3)))),2);
%regularization term
J_temp=sum(Theta1.^2);
J_r=sum(J_temp(2:end),2);
J_temp=sum(Theta2.^2);
J_r=J_r+sum(J_temp(2:end),2);
%sum up
J=J+J_r*lambda/(2*m);
...
%size(z2)
%size(d3)
d2=Theta2'*d3.*sigmoidGradient([ones(1,m);z2]);%26*5000
%size(d2)
Theta2_grad=d3*[ones(1,m);a2]';
Theta1_grad=d2(2:end,:)*[ones(m,1),X];
